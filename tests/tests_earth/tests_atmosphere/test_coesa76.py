"""tests related to module coesa76 in sub-package atmosphere of earth"""
...
def test_outside_altitude_range_coesa76():
    ...
    assert "ValueError: Geometric altitude must be in range [0.0 km, 1000.0 km]" in excinfo.exconly()
...
def test_sound_speed_over_86km():
    ...
    assert "ValueError: Speed of sound in COESA76 has just been implemented up to 86km." in excinfo.exconly()
...
def test_viscosity_over_86km():
    ...
    with pytest.raises(ValueError) as excinfo:
        coesa76.viscosity(z)
    assert "ValueError: Dynamic Viscosity in COESA76 has just been implemented up to 86km." in excinfo.exconly()
...
def test_conductivity_over_86km():
    ...
    assert "ValueError: Thermal conductivity in COESA76 has just been implemented up to 86km." in excinfo.exconly()
