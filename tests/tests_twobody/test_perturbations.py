...
def test_J3_propagation_Earth(test_params):
    ...
    def f_combined(t0, u_, k):
        du_kep = func_twobody(t0, u_, k)
        ax, ay, az = J2_perturbation(t0, u_, k, J2=Earth.J2.value, R=Earth.R.to_value(u.km)) + J3_perturbation(
    ...
    ecc_values_J2 = np.array([rv2coe(k, ri, vi)[1] for ri, vi in zip(r_J2.to(u.km).value, v_J2.to(u.km / u.s).value)])
    ecc_values_J3 = np.array([rv2coe(k, ri, vi)[1] for ri, vi in zip(r_J3.to(u.km).value, v_J3.to(u.km / u.s).value)])
    decc_max = np.max(np.abs(ecc_values_J2 - ecc_values_J3))

    inc_values_J2 = np.array([rv2coe(k, ri, vi)[2] for ri, vi in zip(r_J2.to(u.km).value, v_J2.to(u.km / u.s).value)])
    inc_values_J3 = np.array([rv2coe(k, ri, vi)[2] for ri, vi in zip(r_J3.to(u.km).value, v_J3.to(u.km / u.s).value)])
...
def test_atmospheric_drag_exponential():
    ...
    assert_quantity_allclose(norm(rr[0].to(u.km).value) - norm(r0), dr_expected, rtol=1e-2)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            sun_leo,
            marks=pytest.mark.skip(reason="here agreement required rtol=1e-10, too long for 200 days"),
        ),
    ],
)
def test_3rd_body_Curtis(test_params):
    ...
    for ri, vi in zip(rr.to_value(u.km), vv.to_value(u.km / u.s)):
        angles = Angle(rv2coe(Earth.k.to_value(u.km**3 / u.s**2), ri, vi)[2:5] * u.rad)  # inc, raan, argp
...
def test_solar_pressure(t_days, deltas_expected, sun_r):
    ...
    for ri, vi in zip(rr.to(u.km).value, vv.to(u.km / u.s).value):
        ...
        delta_raans.append((orbit_params[3] * u.rad).to(u.deg).value - initial.raan.value)
        delta_argps.append((orbit_params[4] * u.rad).to(u.deg).value - initial.argp.value)
